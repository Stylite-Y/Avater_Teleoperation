...
//#define K_SBUS_PORT_MC "/dev/ttyUSB0"
...
int init_sbus(int is_simulator) {
    ...
    printf("[RT SUBS] success init sbus\n");
    return fd1;
}
...
static float scale_joystick2(uint16_t in) {
    return (in - 353) * 2.f / (1694.f - 353.f) - 1.f;
}

static WflySwitchState map_switch2(uint16_t in) {
    switch (in) {
        case 1694:
            return WflySwitchState::WFLY_SWITCH_DOWN;
        case 1024:
            return WflySwitchState::WFLY_SWITCH_MIDDLE;
        case 353:
            return WflySwitchState::WFLY_SWITCH_UP;
        default:
            // printf("[SBUS] switch returned bad value %d\n", in);
            return WflySwitchState::WFLY_SWITCH_UP;
    }
...
void update_wfly_et12(Wfly_ET12_data *data) {
    pthread_mutex_lock(&sbus_data_m);
    data->left_stick[0] = scale_joystick2(channel_data[0]);
    data->left_stick[1] = scale_joystick2(channel_data[1]);
    data->right_stick[0] = scale_joystick2(channel_data[3]);
    data->right_stick[1] = scale_joystick2(channel_data[2]);
    data->SA = map_switch2(channel_data[4]);
    data->SB = map_switch2(channel_data[5]);
    data->SC = map_switch2(channel_data[6]);
    data->SD = map_switch2(channel_data[7]);
    data->SE = map_switch2(channel_data[8]);
    data->SF = map_switch2(channel_data[9]);
    data->SG = map_switch2(channel_data[10]);
    data->SH = map_switch2(channel_data[11]);
    pthread_mutex_unlock(&sbus_data_m);
}
