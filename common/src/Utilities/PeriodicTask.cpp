...
void PeriodicTask::printStatus(FILE *FSPOINTER) {
    if (!_running) return;
//    if (isSlow()) {
//        printf_color(PrintColor::Red, "|%-20s|%6.4f|%6.4f|%6.4f|%6.4f|%6.4f\n",
//                     _name.c_str(), _lastRuntime, _maxRuntime, _period,
...
//               _lastRuntime, _maxRuntime, _period, _lastPeriodTime, _maxPeriod);
//    }
    fprintf(FSPOINTER, "|%-20s|%6.4f|%6.4f|%6.4f|%6.4f|%6.4f\n", _name.c_str(),
            _lastRuntime, _maxRuntime, _period, _lastPeriodTime, _maxPeriod);
}
...
//#else
//    FILE *FSPOINTER;
//    FSPOINTER = fopen("./log.txt", "at");
//    time_t t;
//    time(&t);
//    fprintf(FSPOINTER, "\n\nTime: %s", ctime(&t));
//    fprintf(FSPOINTER,"\n----------------------------TASKS----------------------------\n");
//    fprintf(FSPOINTER,"|%-20s|%-6s|%-6s|%-6s|%-6s|%-6s\n", "name", "rt", "rt-max", "T-des",
//           "T-act", "T-max");
//    fprintf(FSPOINTER,"-------------------------------------------------------------\n\n");
//    for (auto &task : _tasks) {
//        task->printStatus(FSPOINTER);
//        task->clearMax();
//    }
//    fclose(FSPOINTER);
//#endif
