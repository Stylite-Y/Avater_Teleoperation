...
void LegController<T>::updateCommand(SpiCommand *spiCommand)
{
  for (int leg = 0; leg < 4; leg++)
  {
    ...
    // datas[leg].tauEstimate = 0

    // spiCommand->flags[leg] = _legsEnabled ? 1 : 0;
    spiCommand->flags[leg] = _legsEnabledArray[leg] ? 1 : 0;
    // std::cout << "Leg " << leg
    //           << " tau_abad is: " << datas[leg].tauEstimate[0]
    //           << " tau_hip is: " << datas[leg].tauEstimate[1]
    //           << " tau_knee is: " << datas[leg].tauEstimate[2] << std::endl;
  }
}
