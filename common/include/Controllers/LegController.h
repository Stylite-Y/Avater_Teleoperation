...
public:
  LegController(Quadruped<T> &quad) : _quadruped(quad) {
    for (auto &data : datas)
      data.setQuadruped(_quadruped); // data 的数据类型是LegControllerData
  }
  ...
  /*!
   *Set the enable flag of each leg together
   */
  void setEnabled(bool enabled) {
    _legsEnabled = enabled;
    for (int i = 0; i < 4; i++)
      _legsEnabledArray[i] = enabled;
  };
  /*!
   *Independently set the enable flag of each leg
   */
  void setEnabled(bool enabled[4]) {
    for (int i = 0; i < 4; i++)
      _legsEnabledArray[i] = enabled[i];
  }
  ...
  bool _legsEnabledArray[4] = {false, false, false, false};
  T _maxTorque = 0;
};
