...
remote_controller rc_controller2;
...
void get_rc_control_settings2(void *settings) {
    pthread_mutex_lock(&lcm_get_set_mutex);
    v_memcpy(settings, &rc_controller2, sizeof(remote_controller));
    pthread_mutex_unlock(&lcm_get_set_mutex);
}

void sbus_packet_complete2() {
//    printf("run sbus3\n");
    Wfly_ET12_data data;
    update_wfly_et12(&data);
    rc_controller2.VxDes = data.right_stick[1];
    rc_controller2.VyDes = data.right_stick[0];
    rc_controller2.OmegaDes = data.left_stick[0];
    rc_controller2.StandUpEnable = (data.SH == WFLY_SWITCH_DOWN);
    rc_controller2.SitDownEnable = (data.SF == WFLY_SWITCH_DOWN);
    rc_controller2.MotorEnable = (data.SA == WFLY_SWITCH_DOWN);
//    rc_controller2.AiModeEnable = (data.SB == WFLY_SWITCH_DOWN);
    rc_controller2.AiModeEnable = (data.SE == WFLY_SWITCH_DOWN);
}
