...
void Simulation::highLevelControl() {
    ...
    // send contact force to robot
    // _sharedMemory().simToRobot.contact_force_z << 0.0, 0.0, 0.0, 0.0;
    // wooden modify, report simulator's force information to robotRunner
    for (size_t leg = 0; leg < 4; leg++) {
        size_t gcID = _simulator->getModel()._footIndicesGC.at(leg);
        _sharedMemory().simToRobot.contact_force_z(leg) = _simulator->getContactForce(gcID)[2];
    }
